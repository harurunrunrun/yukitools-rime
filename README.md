...
ジャッジコード・解説・テストケース・提出を管理するPython製CLIです。

実装の基準として、Rime (`bce5de3`) と yukicoder_tools (`97555b9`) を参照しています。
...
| yukitools-rime submit [SOLUTION] | Rime の解答を提出 |
...
new は PROJECT 直下へ問題を作り、問題設定、問題文、登録済み generator と judge を
...
reference_solution、rime_options、既存の src、prefix、rime_kind は保持します。
remote に generator または judge がない場合はローカル宣言とソースを削除せず
...
取得します。solution は既存提出を想定解として登録または解除します。testcases は
...
TESTSET には generator と judge を最大 1 個ずつ置きます。
...
同期だけできます。rime_options は Rime/Rime Plus の directive へ渡されます。
...
非空の通常ファイルでなければなりません。名前は ASCII 英数字、ピリオド、
アンダースコアだけを許可し、空名、.、..、先頭ピリオド、パス区切りを拒否します。
